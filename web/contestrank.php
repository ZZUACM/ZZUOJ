<?php
	...
	for ($i = 0; $i < $rows_cnt; $i++){
		...
		if(time() < $end_time && $lock < strtotime($row['in_date'])) {
			$U[$user_cnt]->Add($row['num'],
				strtotime($row['in_date']) - $start_time,0);
		} else {
			$U[$user_cnt]->Add($row['num'],
				strtotime($row['in_date']) - $start_time,
				intval($row['result']));
		}
	}
	if(!$OJ_MEMCACHE) {
	...
	$first_blood = array();
	for($i = 0; $i < $pid_cnt; $i++){
		$sql = "select user_id from solution where contest_id=$cid".
			"and result=4 and num=$i order by in_date limit 1";
		$result = mysql_query($sql);
